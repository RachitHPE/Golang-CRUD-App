...
import (
	...
	config "crudApplication/internal/pkg/config"
	controller "crudApplication/internal/pkg/controller"
	database "crudApplication/internal/pkg/database"
	apihandler "crudApplication/internal/pkg/handler"
	log "crudApplication/internal/pkg/log"
...
func (app *AppServerBase) setupAPIWithRouter(ctx context.Context) {
	...
	dbInitializer := database.Init(dbConfig)
	dbHandler := database.NewDBHandler(dbInitializer)

	resourceController := controller.NewController(dbHandler)

	rsrcApiHandler := apihandler.NewAPIHandler(resourceController)

	app.Router.GET("/books", rsrcApiHandler.GetBooks)
	app.Router.POST("/books", rsrcApiHandler.CreateBook)
	app.Router.GET("/books/:id", rsrcApiHandler.GetBookByID)
	app.Router.PUT("/books/:id", rsrcApiHandler.UpdateBook)
	app.Router.DELETE("/books/:id", rsrcApiHandler.DeleteBook)
}
